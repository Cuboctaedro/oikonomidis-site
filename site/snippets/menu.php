<nav class="">
    <button class="block lg:hidden h-6 w-6 bg-transparent border-0 relative z-50" type="button" data-menu-burger>
    ...
    <ul class="flex flex-col lg:flex-row items-center justify-center lg:justify-end absolute lg:static left-0 right-0 h-screen lg:h-auto z-40 bg-white lg:bg-transparent " data-menu-list>
        <?php foreach ($pages->listed() as $item) : ?>
            <?php if ($item->intendedTemplate()->name() === 'works'): ?>
                <li class="lg:pl-8 relative">
                    <button type="button" class="h-8 flex items-center <?= $item->isOpen() ? ' font-bold' : '' ?>" data-submenu-open><?= $item->title() ?></button>
